...
def prepare_4d_attention_mask_bidirect(attention_mask_with_indices: "torch.Tensor", dtype: "torch.dtype", input_ids: "torch.Tensor", image_token_id, video_token_id) -> "torch.Tensor":
    r"""Expand 2d attention mask to 4d attention mask.
    ...
    # Create full attention for image pad
    # TODO: support video token. Currently, all tokens of a video can attend to each other. Video token should only attend to tokens within the same frame
    input_ids_bool = attention_mask_with_indices * ((input_ids == image_token_id) | (input_ids == video_token_id)).long()
    ids = input_ids_bool.unsqueeze(1).unsqueeze(2)
    ids_t = input_ids_bool.unsqueeze(1).unsqueeze(3)
    visual_mask = (ids == ids_t).to(dtype=torch.bool)
    non_image_mask = (input_ids_bool != 0).unsqueeze(1).unsqueeze(2)
    visual_mask = visual_mask & non_image_mask
    ...
    # Create a lower triangular mask.
    tril_mask = torch.tril(torch.ones((seq_len, seq_len), dtype=torch.bool))
    attention_mask_4d = (indices == indices_t) & non_padding_mask & tril_mask | visual_mask
    # Invert the attention mask.
    attention_mask_4d = torch.where(attention_mask_4d, zero_tensor, min_dtype)
    return attention_mask_4d
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        if self.get_rope_func is not None:
            ...
            if getattr(self.model.config, "model_type", None) in ["qwen2_5_omni_thinker", "qwen3_omni_moe_thinker"]:
                ...
                features["rope_deltas"] = features["rope_deltas"].contiguous()
...
@dataclass
class SFTDataCollatorWith4DAttentionMaskBidirection(MultiModalDataCollatorForSeq2Seq):
    r"""Data collator for 4d attention mask."""
    ...
    video_token_id: int = 151654  # default to -100, which is ignored

    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        features = super().__call__(features)
        if self.block_diag_attn and self.attn_implementation != "flash_attention_2":
            features["attention_mask"] = prepare_4d_attention_mask_bidirect(features["attention_mask"], self.compute_dtype, features["input_ids"], self.image_token_id, self.video_token_id)
