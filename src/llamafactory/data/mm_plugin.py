...
class LaverPlugin(BasePlugin):
    ...
    def _get_mm_inputs(
        ...
        if len(images) != 0:
            ...
            num_images, num_channels, height, width = image_inputs['pixel_values'].shape
            image_grid_thw = []
            for i in range(num_images):
                image_grid_thw.append(torch.tensor([1, height // processor.patch_size, width // processor.patch_size]))
    ...
    def process_messages(
        ...
        for message in messages:
            content = message["content"]
            while IMAGE_PLACEHOLDER in content:
                content = content.replace(IMAGE_PLACEHOLDER, f"<|vision_start|>{self.image_token * image_seqlen}<|vision_end|>", 1)
                num_image_tokens += 1
