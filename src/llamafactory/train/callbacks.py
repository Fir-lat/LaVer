...
class TeacherUpdateCallback(TrainerCallback):
    """Callback to update teacher model with EMA after each training step"""
    
    def __init__(self, model, finetuning_args: "FinetuningArguments"): 
        self.model = model
        self.rampup_steps = 0

        self.momentum = finetuning_args.momentum
        self.momentum_strategy = finetuning_args.momentum_strategy
        self.mask_ratio = finetuning_args.mask_ratio
        self.mask_strategy = finetuning_args.mask_strategy
        self.global_mask_ratio = finetuning_args.global_mask_ratio
        self.lambda_laver = finetuning_args.lambda_laver
        self.lambda_gram = finetuning_args.lambda_gram
        self.ema_steps = finetuning_args.ema_steps
        self.ema_ratio = finetuning_args.ema_ratio
    ...
    def on_step_begin(self, args, state, control, **kwargs):
        ...
        self.model.teacher_vision_head.eval()
    
    @override
    def on_step_end(self, args, state, control, **kwargs):
        """Update teacher model with EMA after each step"""
        self.rampup_steps = state.max_steps
        if self.ema_steps == None:
            self.ema_steps = max(int(self.rampup_steps * self.ema_ratio), 1)
        self.step = state.global_step

        if self.step <= self.rampup_steps:
            if self.momentum_strategy == "cosine":

                momentum = 1 - (1 - self.momentum) * (math.cos(math.pi * self.step / self.rampup_steps) + 1) / 2 
            elif self.momentum_strategy == "constant":
                momentum = self.momentum
            else:
                raise ValueError(f"Invalid momentum strategy: {self.momentum_strategy}")
            if self.mask_strategy == "cosine":

                mask_ratio = ((- math.cos(math.pi * self.step / self.rampup_steps) + 1) / 2) * self.mask_ratio
            elif self.mask_strategy == "constant":
                if self.step < self.rampup_steps * 0.125:
                    mask_ratio = 0.0
                else:
            ...
            lambda_laver = self.lambda_laver 
            lambda_gram = self.lambda_gram
        else:
            momentum = self.momentum 
            mask_ratio = self.mask_ratio
            lambda_laver = self.lambda_laver
        ...
        self.model.global_mask_ratio = self.global_mask_ratio
        self.model.lambda_laver = lambda_laver
        self.model.lambda_gram = lambda_gram

        if self.step % self.ema_steps == 0:
            model.update_teacher(self.current_momentum)
...
class LogCallback(TrainerCallback):
    ...
    def on_log(self, args: "TrainingArguments", state: "TrainerState", control: "TrainerControl", **kwargs):
        ...
        logs = dict(
            ...
            loss_log=state.log_history[-1].get("loss_log"),
            loss_mim_log=state.log_history[-1].get("loss_mim_log"),
            similarity_log=state.log_history[-1].get("similarity_log"),
        )
