...
def load_config(model_args: "ModelArguments") -> "PretrainedConfig":
    ...
    if os.path.exists(config_path):
        ...
        if model_args.use_laver == True: # or config_dict["model_type"] == "llava1_5_qwen_mim":
            return LaverConfig.from_pretrained(model_args.model_name_or_path, **init_kwargs)
        elif config_dict["model_type"] == "llava" or config_dict["model_type"] == "laver":

            return LlavaConfig.from_pretrained(model_args.model_name_or_path, **init_kwargs)
        else:
...
def load_model(
    ...
    ####
    if hasattr(config, "image_full_attn"):
        if model_args.image_full_attn == False:
            config.image_full_attn = False
        else:
            config.image_full_attn = config.image_full_attn | model_args.image_full_attn 
    else:
        config.image_full_attn = model_args.image_full_attn
    if "text_config" in config:
        config.text_config.image_full_attn = model_args.image_full_attn
    ...
    if model is None and not lazy_load:
        ...
        if model_args.mixture_of_depths == "load":
            ...
            elif isinstance(config, LlavaConfig):
                load_class = LlavaForConditionalGeneration
            elif isinstance(config, LaverConfig):
                load_class = LaverForConditionalGeneration
            else:
