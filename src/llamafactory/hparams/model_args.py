...
class ModelArguments(
    ...
    patch_size: Optional[int] = field(
        default=16,
        metadata={"help": "The patch size for visual inputs: 16 * 16 or 32 * 32. Please be the same as in preprocessor.json"},
    )

    image_full_attn: Optional[dict] = field(
        default=True,
        metadata={"help": "Whether to use full attention for image tokens. Default is False."},
    )

    use_laver: bool = field(
        default=False,
        metadata={"help": "Whether or not to use the MIM loss."},
    )
