...
class FinetuningArguments(
    ...
    ### laver related arguments

    mask_ratio: float = field(
        default=0.0,
        metadata={"help": "The ratio of the image tokens to be masked."},
    )

    mask_strategy: str = field(
        default="cosine",
        # choices=["cosine", "constant"],
        metadata={"help": "The strategy of the mask ratio."},
    )

    global_mask_ratio: float = field(
        default=0.0,
        metadata={"help": "The ratio of the global image tokens to calculate the ibot loss."},
    )

    momentum: float = field(
        default=1.0,
        metadata={"help": "The momentum of the ibot loss."},
    )

    momentum_strategy: str = field(
        default="cosine",
        # choices=["cosine", "constant"],
        metadata={"help": "The strategy of the momentum."},
    )
    
    lambda_laver: float = field(
        default=1.0,
        metadata={"help": "The lambda of the ibot loss."},
    )

    lambda_gram: float = field(
        default=2.0,
        metadata={"help": "The lambda of the gram loss."},
    )

    ema_steps: int = field(
        default=None,
        metadata={"help": "Update the teacher model every ema_steps steps."},
    )

    ema_ratio: float = field(
        default=0.01,
        metadata={"help": "The ema ratio is used to calculate the ema_steps: ema_steps = total_steps * ema_ratio, if ema_steps is None"},
    )
